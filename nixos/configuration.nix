...

{
  ...
  # ── Swap ────────────────────────────────────────────────────────────────
  # 7.6 GB of RAM with no swap left no cushion: under pressure the kernel
  # kills processes rather than paging, which is the likely cause of VS Code
  # dying repeatedly. Declared here rather than in hardware-configuration.nix,
  # which nixos-generate-config rewrites; list options merge across modules,
  # so this joins the empty list already declared there.
  swapDevices = [
    {
  ...
  environment.systemPackages = with pkgs; [
    # Core utils
    git curl wget ripgrep fd bat eza fzf zoxide jq unzip
    # Languages
    python3
    ...
    catppuccin-cursors      # every flavour, in every accent
    bibata-cursors          # Modern Ice / Modern Classic / Original
    phinger-cursors         # dark, light and mixed
    capitaine-cursors
    nordzy-cursor-theme
    rose-pine-cursor
    simp1e-cursors
    ...
    # ── Neovim toolchain ────────────────────────────────────────────────
    # Mason cannot be used on NixOS: it fetches dynamically linked binaries
    # that will not run against the Nix store. Everything Neovim shells out
    # to therefore lives here instead, and nvim's config installs no binaries.
    gcc             # nvim-treesitter compiles each parser from source
    tree-sitter     # ...and its `main` branch drives the CLI to do it
    # Language servers
    ...
    stylua nixfmt-rfc-style shfmt ruff
    # Linters
    shellcheck
    # Wayland tooling
    ...
    # be 64 builds.
  ] ++ (map (v: pkgs.catppuccin-gtk.override {
    accents = [ "mauve" ];
    variant = v;
    size = "standard";
